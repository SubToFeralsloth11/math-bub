/**
 * Extract OneNote notebook using a persistent browser profile.
 *
 * Uses the profile saved by agent-browser at /tmp/onenote-profile,
 * which has the authenticated session. No login required.
 *
 * Usage: bun run .agents/skills/onenote-extraction/scripts/extractWithProfile.ts
 *
 * @author John Grimes
 */
...
const PROFILE_DIR = "/tmp/onenote-profile";
...
async function main() {
  ...
  console.log("Launching persistent context from profile...");
  ...
  // Handle any post-navigation prompts.
  await handlePostNavigation(page);
  ...
  for (const sectionName of sections) {
    ...
    if (!wopiFrame) {
      console.log("  Frame lost; stopping.");
      break;
    }
...
async function handlePostNavigation(page: Page) {
...
async function clickSection(frame: Frame, name: string): Promise<boolean> {
  try {
    const clicked = await frame.evaluate((sn: string) => {
      const trees = document.querySelectorAll(
        '[role="navigation"] [role="tree"]',
      );
      const st = trees[0];
      if (!st) return false;
      for (const item of st.querySelectorAll('[role="treeitem"]')) {
        if (item.textContent?.trim() === sn) {
...
async function getPages(frame: Frame): Promise<string[]> {
  try {
    const pages = await frame.evaluate(() => {
      ...
      return [...pt.querySelectorAll('[role="treeitem"]')]
...
async function clickPage(frame: Frame, name: string): Promise<boolean> {
  ...
  try {
    await frame.evaluate((tn: string) => {
      for (const item of document.querySelectorAll('[role="treeitem"]')) {
        if (item.textContent?.trim() === tn) {
          (item as HTMLElement).click();
          return;
        }
      }
